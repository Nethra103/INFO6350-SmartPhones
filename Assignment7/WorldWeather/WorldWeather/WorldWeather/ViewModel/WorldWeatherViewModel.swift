...
class WorldWeatherViewModel{
    ...
    func getFiveDayConditions(_ url: String) -> Promise<[ModelFiveDayForecast]>{
        return Promise<[ModelFiveDayForecast]> { seal -> Void in
            ...
            getAFResponseJSON(url).done { json in
                var fivedayarr = [ModelFiveDayForecast]()
                for weekday in json["DailyForecasts"].arrayValue {
                    
                    let date = weekday["Date"].stringValue.split(separator:":")
                    let index = date[0].firstIndex(of: "T")
                    let dayOfWeek = getDayOfWeek(String(date[0].prefix(upTo:index!)))!
                    let max = weekday["Temperature"]["Maximum"]["Value"].intValue
                    let maxImg = weekday["Day"]["Icon"].intValue
                    let min = weekday["Temperature"]["Minimum"]["Value"].intValue
                    let minImg = weekday["Night"]["Icon"].intValue
                    let forecast = ModelFiveDayForecast(weekday: dayOfWeek, max: max, maxImg: "\(maxImg)-s", min: min, minImg: "\(minImg)-s")
                    fivedayarr.append(forecast)
                }
                seal.fulfill(fivedayarr)
